 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class $n{constructor(e,t,n){this.source=e,this.path=t,this.children=n,this.type=Wn.MERGE}operationForChild(e){if(At(this.path)){const t=this.children.subtree(new It(e));return t.isEmpty()?null:t.value?new zn(this.source,Et(),t.value):new $n(this.source,Et(),t)}return i(St(this.path)===e,"Can't get a merge for a child not on the path of the operation"),new $n(this.source,Nt(this.path),this.children)}toString(){return"Operation("+this.path+": "+this.source.toString()+" merge: "+this.children.toString()+")"}}
/**
 * @license
 * Copyright 2017 Google LLC
 *
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */();class zr{constructor(e,t,n){this.id=e,this.name=t,this.commentText=n,this.displayTask()}displayTask(){const e=document.createElement("h3");e.id=this.id,e.innerText=`User: ${this.name}- Comment: ${this.commentText}`,document.querySelector("#comment-container").append(e)}clearDOM(){document.querySelector(`#${this.id}`).remove()}}
/**
 * @license
 * Copyright 2020 Google LLC
 *
 ...
 * limitations under the License.
 */
ie("firebase","9.6.9","app");const $r=function(e=function(e="[DEFAULT]"){const t=Q.get(e);if(!t)throw te.create("no-app",{appName:e});return t}(),t){return ee(e,"database").getImmediate({identifier:t})}(function(e,t={}){if("object"!=typeof t){t={name:t}}const n=Object.assign({name:"[DEFAULT]",automaticDataCollectionEnabled:!1},t),i=n.name;if("string"!=typeof i||!i)throw te.create("bad-app-name",{appName:String(i)});const s=Q.get(i);if(s){if(S(e,s.options)&&S(n,s.config))return s;throw te.create("duplicate-app",{appName:i})}const r=new W(i);for(const e of X.values())r.addComponent(e);const o=new ne(e,n,r);return Q.set(i,o),o}({apiKey:"AIzaSyBtlvjJwGNzNGOY40SpCEM2zaEa_2a5H5s",authDomain:"chat-4aacb.firebaseapp.com",databaseURL:"https://chat-4aacb-default-rtdb.europe-west1.firebasedatabase.app",projectId:"chat-4aacb",storageBucket:"chat-4aacb.appspot.com",messagingSenderId:"924350170807",appId:"1:924350170807:web:8c0a71ab047031ea402f32"})),Vr=Rr($r,"/Comments"),Yr=document.querySelector("#user-name");let Gr=[];document.querySelector("#start-button").addEventListener("click",(()=>{document.querySelector("#welcome-massage").innerText=`Hello ${Yr.value} if u want to delete your old comments click on it !!!`,Wr(Vr,(e=>{document.querySelector("#comment-container").style.display="inline-block";const t=e.val();for(const e of Gr)e.clearDOM();Gr=[];for(const e in t)Gr.push(new zr(e,t[e].name,t[e].comment));for(const e of Gr)e.name==Yr.value&&(document.querySelector(`#${e.id}`).style.color="white",document.querySelector(`#${e.id}`).addEventListener("click",(()=>{Ar(Rr($r,"/Comments/"+e.id))})));document.querySelector("#start-div").style.display="none"}))})),document.querySelector("#add").addEventListener("click",(e=>{if(e.preventDefault(),Gr.length>24){Ar(Rr($r,"/Comments/"+Gr[0].id))}const t={comment:document.querySelector("#new-comment").value,name:Yr.value},n={};n[function(e,t){e=R(e),Ys("push",e._path),Bs("push",t,e._path,!0);const n=ir(e._repo),i=wn(n),s=Dr(e,i),r=Dr(e,i);let o;return o=null!=t?Mr(r,t).then((()=>r)):Promise.resolve(r),s.then=o.then.bind(o),s.catch=o.then.bind(o,void 0),s}(Vr).key]=t,Or(Vr,n)}));
//# sourceMappingURL=index.432db3d6.js.map
